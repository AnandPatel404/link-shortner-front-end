...
export const LoginLogTable = (props) => {
			...
			<tbody>
				{props.activity && props.activity.length > 0
					? props.activity.map((item, idx) => {
							return (
								<tr key={idx}>
									<td className="tb-col-os">{item.browser_name}</td>
									...
									<td className="tb-col-time">
										<span className="sub-text">
											...
											<span className="d-none d-sm-inline-block">{new Date(item.createdAt).toLocaleTimeString()}</span>
										</span>
									</td>
								</tr>
							);
